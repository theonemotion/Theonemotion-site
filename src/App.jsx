...
import { Image, Box, FormControl, FormLabel, Input, Button, Text, Heading } from '@chakra-ui/react'
...
function App() {
  ...
  return (
    <Box className="App" margin='0' padding='0'>
      {/*
      <Navbar />
      <Image src='src/assets/Logo.png' className='hero-logo'  pos='absolute' left='40%' top='0px' />
      <Box w='100%' h='920px' backgroundColor='teal.500'>
        <Image src='src/assets/down.svg' w='50px' pos='absolute' top='800px' left='50%' onClick={() => {hero.scrollIntoView({behavior: 'smooth', block: 'center'})}}></Image>
      </Box>
      <Information />
      <FormControl backgroundColor='#171717' color='white' display='flex' flexDir='column' justifyContent='center' textAlign='center' alignItems='center'>
        <Image src='src/assets/Logo.png' className='toWhite' w='400px' marginBottom='-100px' marginTop='-100px'/>
      ...
      */}
      ...
      <Parallax speed={40}>
        <Box className='wrapper'>
          <video className='hero-video' src='src/assets/PotentialFinalDraft.mp4' controls muted autoPlay loop/>
          <Image src='src/assets/LogoForWebsuteNEW.png' className='logo'/>
          <Image src='src/assets/down.svg' className='toWhite' w='5%' pos='absolute' top='80%' left='45%' onClick={() => {hero.scrollIntoView({behavior: 'smooth', block: 'center'})}}/>
        </Box>
      </Parallax>
      <Information />
    </Box>
